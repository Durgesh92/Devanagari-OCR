...
i16=imresize((edu_imgcrop(imread('21.bmp'))),[50 50]);

i17=imresize((edu_imgcrop(imread('22.bmp'))),[50 50]);

i18=imresize((edu_imgcrop(imread('34.bmp'))),[50 50]);

i19=imresize((edu_imgcrop(imread('w1.bmp'))),[50 50]);

i20=imresize((edu_imgcrop(imread('w2.bmp'))),[50 50]);
i21=imresize((edu_imgcrop(imread('43.bmp'))),[50 50]);
...
le=[s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21];
t=eye(21);
...
%net = train(net,le,t);
%Y = sim(net,le);
net = newff(minmax(le),[40 21],{'logsig' 'logsig'},'traingdx');
